...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    } elseif(!filter_var($input_rupess, FILTER_VALIDATE_REGEXP, array("options"=>array("regexp"=>"/^[0-9\s]+$/")))){
    ...
    if(empty($acc_err) && empty($ifsc_err) && empty($rupess_err) ){
        ...
        $sql = "INSERT INTO bank (accnumber,IFSC,rupess) VALUES (?, ?,?)";
         
        if($stmt = mysqli_prepare($link, $sql)){
            
            mysqli_stmt_bind_param($stmt, "isi", $param_acc, $param_ifsc,$param_rupess,);
...
<form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
    ...
    <div class="label">
		...
		<input type="number" name="rupees" value="<?php echo $rupess; ?>" required>
